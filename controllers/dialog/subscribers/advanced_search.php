...
class AdvancedSearch extends AdvancedSearchController
{
    ...
    public function getSearchPresets()
    {
        ...
        if (is_object($em)) {
            return $em->getRepository(SavedSubscriberSearch::class)->findAll();
        }
    }
    ...
    {
        return $this->app->make('url')->to('/dashboard/simple_newsletter/subscribers', 'advanced_search');
    }
    ...
    {
        return $this->app->make('url')->to('/dashboard/simple_newsletter/subscribers', 'preset', $search->getID());
    }

    public function getSavedSearchDeleteURL(SavedSearch $search)
    {
        return (string)Url::to('/ccm/system/dialogs/subscribers/advanced_search/preset/delete?presetID=' . $search->getID());
    }

    public function getSavedSearchEditURL(SavedSearch $search)
    {
        return (string)Url::to('/ccm/system/dialogs/subscribers/advanced_search/preset/edit?presetID=' . $search->getID());
    }

    public function getCurrentSearchBaseURL()
    {
        return (string)Url::to('/ccm/system/search/subscribers/current');
    }
