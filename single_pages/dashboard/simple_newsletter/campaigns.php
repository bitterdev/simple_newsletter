...
use Bitter\SimpleNewsletter\Campaign\Search\Result\Column;
use Bitter\SimpleNewsletter\Campaign\Search\Result\Result;
use Bitter\SimpleNewsletter\Campaign\Search\Result\Item;
use Bitter\SimpleNewsletter\Campaign\Search\Result\ItemColumn;
use Bitter\SimpleNewsletter\Campaign\Menu;
...
<div id="ccm-search-results-table">
    <table class="ccm-search-results-table" data-search-results="campaigns">
        <thead>
        <tr>
        ...
        <?php foreach ($result->getItems() as $item) { ?>
            ...
            $campaign = $item->getItem();
            $editUrl = Url::to('/dashboard/simple_newsletter/campaigns', 'update', $campaign->getId());
            if ($campaign->getState() != CampaignState::DRAFT) {
                $editUrl = "#";
            }
            ?>
            <tr data-details-url="<?php echo $editUrl ?>">
                <td class="ccm-search-results-checkbox">
                    <?php if ($campaign instanceof Campaign) { ?>
                        <!--suppress HtmlFormInputWithoutLabel -->
                        <input data-search-checkbox="individual"
                               type="checkbox"
                               data-item-id="<?php echo $campaign->getId() ?>"/>
                    <?php } ?>
                </td>

                <?php foreach ($item->getColumns() as $column) { ?>
                    <?php /** @var ItemColumn $column */ ?>
                    <?php if ($column->getColumnKey() == 'c.name') { ?>
...
<div style="display: none">
    <div id="ccm-dialog-delete-campaign" class="ccm-ui">
        <p>
