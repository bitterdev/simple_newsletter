...
use Bitter\SimpleNewsletter\MailingList\Search\Result\Column;
use Bitter\SimpleNewsletter\MailingList\Search\Result\Result;
use Bitter\SimpleNewsletter\MailingList\Search\Result\Item;
use Bitter\SimpleNewsletter\MailingList\Search\Result\ItemColumn;
use Bitter\SimpleNewsletter\MailingList\Menu;
...
<div id="ccm-search-results-table">
    <table class="ccm-search-results-table" data-search-results="mailing-lists">
        ...
        <?php foreach ($result->getItems() as $item) { ?>
            ...
            $mailingList = $item->getItem();
            ?>
            <tr data-details-url="<?php echo Url::to('/dashboard/simple_newsletter/mailing_lists', 'update', $mailingList->getId()) ?>">
                <td class="ccm-search-results-checkbox">
                    <?php if ($mailingList instanceof MailingList) { ?>
                        <!--suppress HtmlFormInputWithoutLabel -->
                        <input data-search-checkbox="individual"
                               type="checkbox"
                               data-item-id="<?php echo $mailingList->getId() ?>"/>
                    <?php } ?>
                </td>

                <?php foreach ($item->getColumns() as $column) { ?>
                    <?php /** @var ItemColumn $column */ ?>
                    <?php if ($column->getColumnKey() == 'm.name') { ?>
...
<div style="display: none">
    <div id="ccm-dialog-delete-mailingList" class="ccm-ui">
        <p>
