...
class DashboardCampaignBreadcrumbFactory implements ApplicationAwareInterface
{
    ...
    public function getBreadcrumb(Page $dashboardPage, $mixed = null): BreadcrumbInterface
    {
        ...
        if ($mixed instanceof Campaign) {
            /** @noinspection PhpUnhandledExceptionInspection */
            $item = new Item(
                $this->app->make('url')->to(
                    '/dashboard/simple_newsletter/campaigns', 'update', $mixed->getId()
                ),
                $mixed->getName()
            );
